...
int tcp_open(char *host, char *service, int port)
{
  ...
  */
  ...
  if(service != NULL)
  {
    ...
    {
      warnx("tcp_open: unknown service: %s/tcp\n", service);
      return(-1);
    }
  ...
  else
  {
    if(port <= 0)
    {
      warnx("tcp_open: must specify either service or port");
      return(-1);
    }
  ...
  */
  ...
  else
  {
    ...
    {
      warnx("tcp_open: host name error: %s %s\n", host, host_err_str());
      return(-1);
    }
  ...
  if(port >= 0)
  {
    ...
    {
      warnx("tcp_open: can't create TCP socket\n");
      return(-1);
    }
  }
  else if(port < 0)
  {
    ...
    {
      warnx("tcp_open: can't get a reserved TCP port\n");
      return(-1);
    }
  }
  ...
  */
  ...
  {
    warnx("tcp_open: can't connect to server\n");
    close(fd);
    return(-1);
  }
...
int udp_open(char *host, char *service, int port, int dontconn)
{
  ...
  */
  ...
  if(service != NULL)
  {
    ...
    {
      warnx("udp_open: unknown service: %s/udp\n", service);
      return(-1);
    }
  ...
  else
  {
    if(port <= 0)
    {
      warnx("udp_open: must specify either service or port\n");
      return(-1);
    }
  ...
  */
  ...
  else
  {
    if( (hp = gethostbyname(host)) == NULL )
    {
      warnx("udp_open: host name error: %s %s\n", host, host_err_str());
      return(-1);
    }
  ...
  if(port < 0)
  {
    warnx("udp_open: reserved ports not implemeneted yet\n");
    return(-1);
  }

  if( (fd = socket(AF_INET, SOCK_DGRAM, 0)) < 0 )
  {
    warnx("udp_open: can't create UDP socket\n");
    return(-1);
  }
...
void str_echo(int sockfd)
{
  ...
  while(1)
  {
    if( (n = readline(sockfd, line, MAXLINE)) < 0)
      warnx("str_echo: readline error\n");
    else if(n == 0)
      return;       /* connection closed */
      
    if(writen(sockfd, line, n) != n)
      warnx("str_echo: writen error\n");
  }
}
  ...
  while(fgets(sendline, MAXLINE, fp) != NULL)
  {
    ...
    */
    ...
    if( (n = readline(sockfd, recvline, MAXLINE)) < 0)
      warnx("str_cli: readline error\n");
    recvline[n] = 0;
    fputs(recvline, stdout);
  }
  if(ferror(fp))
    warnx("str_cli: error reading file\n");
}
...
void dg_echo(int sockfd, struct sockaddr *pcli_addr, int maxclilen)
{
  ...
  while(1)
  {
    ...
    if( (n = recvfrom(sockfd, mesg, MAXMESG, 0, pcli_addr, &clilen)) < 0)
      warnx("dg_echo: recvfrom error\n");
      
    if(sendto(sockfd, mesg, n, 0, pcli_addr, clilen) != n)
      warnx("dg_echo: sendto error\n");
  }
}
  ...
  while(fgets(sendline, MAXLINE, fp) != NULL)
    ...
    */
    ...
    if( (n = recvfrom(sockfd, recvline, MAXLINE, 0, NULL, NULL)) < 0)
      warnx("dg_cli: recvfrom error\n");
    recvline[n] = 0;
    fputs(recvline, stdout);
  }
  if(ferror(fp))
    warnx("dg_cli: error reading file\n");
}
