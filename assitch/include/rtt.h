...
struct rtt_struct
{
	...
	float           rtt_srtt;	/* smoothed round-trip time (SRTT),
					 * seconds */
	float           rtt_rttdev;	/* smoothed mean deviation
					 * (Abweichung), sec. */
	short           rtt_nrexmt;	/* #times retransmitted: 0, 1, 2, 3,
					 * ... */
	short           rtt_currto;	/* current retransmit timeout (RTO),
					 * seconds */
	short           rtt_nxtrto;	/* retransmit timeout for next
					 * packet, if nonzero */
